...
export default function BookingSeatSelectionPage() {
  ...
  // Load Showtime & Seat Data
  const loadShowtimeData = useCallback(async () => {
    try {
      setError(null);
      const res = await fetch(`/api/showtimes/${showtimeId}`);
      if (!res.ok) throw new Error("Showtime not found or unavailable");
      const data = await res.json();
      setShowtime(data.showtime);
      setSeats(data.seats || []);
    } catch (err: any) {
      setError(err.message || "Failed to load showtime seat map");
    } finally {
      setLoading(false);
    }
  }, [showtimeId]);
  ...
  useEffect(() => {
    loadShowtimeData();
    // Poll every 10 seconds for real-time seat changes
    const interval = setInterval(loadShowtimeData, 10000);
  ...
  // Minor Units (Cents) Calculation
  const SERVICE_FEE_CENTS = 150; // $1.50 per seat
  const TAX_RATE = 0.05;

  let subtotalCents = 0;
  for (const s of selectedSeatsInfo) {
    let seatPrice = showtime?.basePriceCents || 1500;
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to lock seats. Please try another seat.");
      }

      // Redirect to checkout with newly created booking
      router.push(`/checkout/${data.booking.id}`);
    } catch (err: any) {
      setError(err.message);
      // Reload seat map to get updated statuses
      loadShowtimeData();
    } finally {
      setHolding(false);
    }
  ...
  if (error && !showtime) {
    return (
      <div className="min-h-screen py-24 text-center max-w-md mx-auto space-y-4">
        ...
        <h2 className="text-xl font-bold text-white">Screening Unavailable</h2>
  ...
  return (
    <div className="min-h-screen pb-20 pt-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        {/* Left 2 Cols: Interactive Auditorium Map */}
        <div className="lg:col-span-2 rounded-3xl bg-cinema-card p-6 border border-cinema-border shadow-xl">
          <SeatMap
            seats={seats}
            basePriceCents={showtime?.basePriceCents || 1500}
            selectedSeatIds={selectedSeatIds}
            onToggleSeat={handleToggleSeat}
          />
        </div>

        {/* Right 1 Col: Live Price Breakdown & Hold CTA */}
        <div className="rounded-3xl bg-cinema-card p-6 border border-cinema-border space-y-6 shadow-xl sticky top-24">
          ...
          <div className="space-y-2.5 pt-4 border-t border-slate-800 text-xs text-slate-300">
            ...
            <p className="text-[10px] text-slate-500 text-right">
              Stored as exact minor units ({totalCents}¢)
            </p>
          </div>
