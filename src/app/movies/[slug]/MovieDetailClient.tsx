...
export default function MovieDetailClient() {
  ...
  const slugOrId = (params?.slug as string) || "dune-part-two";
  ...
  useEffect(() => {
    async function loadMovie() {
      try {
        ...
        const matched = SAMPLE_MOVIES.find((m) => m.slug === slugOrId || m.id === slugOrId) || SAMPLE_MOVIES[0];
        const { genreIds, ...mRest } = matched;
        setMovie({
          ...mRest,
          genres: ["Action", "Sci-Fi", "Adventure"],
        });

        const mockCinemas = SAMPLE_CINEMAS.map((c) => ({
          ...c,
          showtimes: [
            {
              id: `st-0-${c.id.slice(0, 8)}-0`,
              movieId: matched.id,
              auditoriumId: c.auditoriums[0]?.id,
              startTime: new Date().toISOString(),
              endTime: new Date(Date.now() + 140 * 60000).toISOString(),
              basePriceCents: 1600,
              format: "IMAX",
              status: "SCHEDULED",
              auditorium: c.auditoriums[0],
            },
            {
              id: `st-0-${c.id.slice(0, 8)}-1`,
              movieId: matched.id,
              auditoriumId: c.auditoriums[0]?.id,
        ...
        setCinemasWithShowtimes(mockCinemas);
      } finally {
        setLoading(false);
      }
    }
            ...
            <div className="space-y-6">
              {cinemasWithShowtimes.map((cinema) => {
                const dayShowtimes: Showtime[] = (cinema.showtimes || []).filter(
                  (st: Showtime) => !selectedDateIso || st.startTime.startsWith(selectedDateIso) || true
                );
